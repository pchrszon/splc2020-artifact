...
echo "nodes; files; translation; lines; states; nodes; construction; checking" > "$RESULTS"

for s in $(seq 2 "$MAX_SIZE"); do
    for f in $(seq 1 "$MAX_FILES"); do
        for t in ma sa comp; do
            model="$BUILD_DIR/${s}_${f}.${t}.prism"
            if [[ -f "$model" ]]; then
                logfile="$LOG_DIR/${s}_${f}.${t}.1.log"
                ...
                echo "${s};" "${f};" "${t};" "${lines};" "${states};" "${nodes};" "${constr};" "${check}" >> "$RESULTS"
            fi
        done
    done
done
